...
durations = cumsum(durations_diff) - durations_diff(1);
...
for t1 = 1:length(times)
   ...
   axes(ha(t1));
   h = imshow(reg(:,:, current+1), [1200, 25000]);
   if current == 0
       hold on;
...
subplot(1,2,2)
imshow(clean_e, [0, 200])
colormap(gca, viridis(256))
c = colorbar;
c.Label.String = '\tau_2 (Min)';
xlim([25 112]);
ylim([21 107]);
export_fig PixelWise.eps -depsc
%%
%% PCA the movie
data1 = data_middle;
data1(isnan(data1)) = 0;
...
[fit_res2, gof2, x2]= fitDouble_Figure(t(data.offset:end), ...
     median_pca, ['PCAMedian_' num2str(comp_id)]);