function [tbl_out] = pairwise_compare_shuffleBoth(tbl, varargin)
...
for i = 1:height(id)
    ...
    if is_string
        for kk=1:length(name1)
            n1 = name1(kk);
            vals = current.tau_values(strcmp(current{:,groupName},n1 ));
    ...
    for b = 1:n_boot
        group_current = Shuffle(group_sub);% group_sub(randperm(length(group_sub)));
        ratios(b) = mean(tau_sub(group_current == 0)) ./ ...
            mean(tau_sub(group_current == 1));
    end
    ratio_true = mean(tau_false) ./ mean(tau_true);
